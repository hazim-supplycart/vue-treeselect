  ...
  methods: {
    ...
    traverseAllNodesDFS: function traverseAllNodesDFS(callback) {
      ...
      this.forest.normalizedOptions.forEach(function (rootNode) {
        _this0.traverseDescendantsDFS(rootNode, callback);
        callback(rootNode);
      });
    },
    ...
    handleLocalSearch: function handleLocalSearch() {
      ...
      var done = function done() {
        return _this1.resetHighlightedOptionWhenNecessary(true);
      };
      if (!searchQuery) {
      ...
      this.traverseAllNodesDFS(function (node) {
        if (node.isBranch) {
          ...
          _this1.$set(_this1.localSearch.countMap, node.id, defineProperty_default()(defineProperty_default()(defineProperty_default()(defineProperty_default()({}, ALL_CHILDREN, 0), ALL_DESCENDANTS, 0), LEAF_CHILDREN, 0), LEAF_DESCENDANTS, 0));
        }
      });
      var lowerCasedSearchQuery = searchQuery.trim().toLocaleLowerCase();
      var splitSearchQuery = lowerCasedSearchQuery.replace(/\s+/g, ' ').split(' ');
      this.traverseAllNodesDFS(function (node) {
        if (_this1.searchNested && splitSearchQuery.length > 1) {
          node.isMatched = splitSearchQuery.every(function (filterValue) {
            return match(false, filterValue, node.nestedSearchLabel);
          });
        } else {
          node.isMatched = _this1.matchKeys.some(function (matchKey) {
            return match(!_this1.disableFuzzyMatching, lowerCasedSearchQuery, node.lowerCased[matchKey]);
          });
        }
        if (node.isMatched) {
          _this1.localSearch.noResults = false;
          node.ancestors.forEach(function (ancestor) {
            return _this1.localSearch.countMap[ancestor.id][ALL_DESCENDANTS]++;
          });
          if (node.isLeaf) node.ancestors.forEach(function (ancestor) {
            return _this1.localSearch.countMap[ancestor.id][LEAF_DESCENDANTS]++;
          });
          if (node.parentNode !== NO_PARENT_NODE) {
            _this1.localSearch.countMap[node.parentNode.id][ALL_CHILDREN] += 1;
            if (node.isLeaf) _this1.localSearch.countMap[node.parentNode.id][LEAF_CHILDREN] += 1;
          }
        }
    ...
    handleRemoteSearch: function handleRemoteSearch() {
      ...
      var done = function done() {
        _this10.initialize();
        _this10.resetHighlightedOptionWhenNecessary(true);
      };
      ...
      this.callLoadOptionsProp({
        ...
        succeed: function succeed(options) {
          ...
          if (_this10.trigger.searchQuery === searchQuery) done();
        },
    ...
    getRemoteSearchEntry: function getRemoteSearchEntry() {
      ...
      this.$watch(function () {
        ...
        if (_this11.trigger.searchQuery === searchQuery) _this11.initialize();
      }, {
        deep: true
      });
      if (searchQuery === '') {
    ...
    normalize: function normalize(parentNode, nodes, prevNodeMap) {
      ...
      var normalizedOptions = nodes.map(function (node) {
        return [_this14.enhancedNormalizer(node), node];
      }).map(function (_ref, index) {
        var _ref2 = slicedToArray_default()(_ref, 2),
          node = _ref2[0],
          raw = _ref2[1];
        _this14.checkDuplication(node);
        _this14.verifyNodeShape(node);
        var id = node.id,
          label = node.label,
        ...
        var isLeaf = !isBranch;
        var isDisabled = !!node.isDisabled || !_this14.flat && !isRootNode && parentNode.isDisabled;
        var isNew = !!node.isNew;
        var lowerCased = _this14.matchKeys.reduce(function (prev, key) {
        ...
        var normalized = _this14.$set(_this14.forest.nodeMap, id, createMap());
        _this14.$set(normalized, 'id', id);
        _this14.$set(normalized, 'label', label);
        _this14.$set(normalized, 'level', level);
        _this14.$set(normalized, 'ancestors', isRootNode ? [] : [parentNode].concat(parentNode.ancestors));
        _this14.$set(normalized, 'index', (isRootNode ? [] : parentNode.index).concat(index));
        _this14.$set(normalized, 'parentNode', parentNode);
        _this14.$set(normalized, 'lowerCased', lowerCased);
        _this14.$set(normalized, 'nestedSearchLabel', nestedSearchLabel);
        _this14.$set(normalized, 'isDisabled', isDisabled);
        _this14.$set(normalized, 'isNew', isNew);
        _this14.$set(normalized, 'isMatched', false);
        _this14.$set(normalized, 'isHighlighted', false);
        _this14.$set(normalized, 'isBranch', isBranch);
        _this14.$set(normalized, 'isLeaf', isLeaf);
        _this14.$set(normalized, 'isRootNode', isRootNode);
        _this14.$set(normalized, 'raw', raw);
        if (isBranch) {
          var isLoaded = Array.isArray(children);
          _this14.$set(normalized, 'childrenStates', _objectSpread(_objectSpread({}, createAsyncOptionsStates()), {}, {
            isLoaded: isLoaded
          }));
          _this14.$set(normalized, 'isExpanded', typeof isDefaultExpanded === 'boolean' ? isDefaultExpanded : level < _this14.defaultExpandLevel);
          _this14.$set(normalized, 'hasMatchedDescendants', false);
          _this14.$set(normalized, 'hasDisabledDescendants', false);
          _this14.$set(normalized, 'isExpandedOnSearch', false);
          _this14.$set(normalized, 'showAllChildrenOnSearch', false);
          _this14.$set(normalized, 'count', defineProperty_default()(defineProperty_default()(defineProperty_default()(defineProperty_default()({}, ALL_CHILDREN, 0), ALL_DESCENDANTS, 0), LEAF_CHILDREN, 0), LEAF_DESCENDANTS, 0));
          _this14.$set(normalized, 'children', isLoaded ? _this14.normalize(normalized, children, prevNodeMap) : []);
          if (isDefaultExpanded === true) normalized.ancestors.forEach(function (ancestor) {
            ancestor.isExpanded = true;
          });
          if (!isLoaded && typeof _this14.loadOptions !== 'function') {
            warning_warning(function () {
              return false;
            }, function () {
            ...
            _this14.loadChildrenOptions(normalized);
          }
        }
    ...
    loadRootOptions: function loadRootOptions() {
      ...
      this.callLoadOptionsProp({
        ...
        isPending: function isPending() {
          return _this15.rootOptionsStates.isLoading;
        },
        start: function start() {
          _this15.rootOptionsStates.isLoading = true;
          _this15.rootOptionsStates.loadingError = '';
        },
        succeed: function succeed() {
          _this15.rootOptionsStates.isLoaded = true;
          _this15.$nextTick(function () {
            _this15.resetHighlightedOptionWhenNecessary(true);
          });
        },
        fail: function fail(err) {
          _this15.rootOptionsStates.loadingError = getErrorMessage(err);
        },
        end: function end() {
          _this15.rootOptionsStates.isLoading = false;
        }
      });
    },
    loadChildrenOptions: function loadChildrenOptions(parentNode) {
      ...
      this.callLoadOptionsProp({
        ...
        isPending: function isPending() {
          return _this16.getNode(id).childrenStates.isLoading;
        },
        start: function start() {
          _this16.getNode(id).childrenStates.isLoading = true;
          _this16.getNode(id).childrenStates.loadingError = '';
        },
        succeed: function succeed() {
          _this16.getNode(id).childrenStates.isLoaded = true;
        },
        fail: function fail(err) {
          _this16.getNode(id).childrenStates.loadingError = getErrorMessage(err);
        },
        end: function end() {
          _this16.getNode(id).childrenStates.isLoading = false;
        }
      });
    },
    ...
    checkDuplication: function checkDuplication(node) {
      var _this17 = this;
      warning_warning(function () {
        return !(node.id in _this17.forest.nodeMap && !_this17.forest.nodeMap[node.id].isFallbackNode);
      }, function () {
        return "Detected duplicate presence of node id ".concat(JSON.stringify(node.id), ". ") + "Their labels are \"".concat(_this17.forest.nodeMap[node.id].label, "\" and \"").concat(node.label, "\" respectively.");
      });
    },
        ...
        if (this.single || this.allowClearingDisabled) {
          this.forest.selectedNodeIds = [];
            ...
            this.forest.selectedNodeIds = this.forest.selectedNodeIds.filter(function (nodeId) {
              return _this18.getNode(nodeId).isDisabled;
            });
          }
        this.buildForestState();
      }
    ...
    _selectNode: function _selectNode(node) {
      ...
      if (this.flat) {
        ...
        if (this.autoSelectAncestors) {
          node.ancestors.forEach(function (ancestor) {
            if (!_this19.isSelected(ancestor) && !ancestor.isDisabled) _this19.addValue(ancestor);
          });
        } else if (this.autoSelectDescendants) {
          this.traverseDescendantsBFS(node, function (descendant) {
            if (!_this19.isSelected(descendant) && !descendant.isDisabled) _this19.addValue(descendant);
          });
        }
        return;
      }
      ...
      if (node.isBranch) {
        this.traverseDescendantsBFS(node, function (descendant) {
          if (!descendant.isDisabled || _this19.allowSelectingDisabledDescendants) {
            _this19.addValue(descendant);
          }
        });
      }
      if (isFullyChecked) {
        ...
        while ((curr = curr.parentNode) !== NO_PARENT_NODE) {
          if (curr.children.every(this.isSelected)) {
            if (this.allowSingleChild) break;else this.addValue(curr);
          } else break;
        }
      }
    },
    _deselectNode: function _deselectNode(node) {
      ...
      if (this.flat) {
        ...
        if (this.autoDeselectAncestors) {
          node.ancestors.forEach(function (ancestor) {
            if (_this20.isSelected(ancestor) && !ancestor.isDisabled) _this20.removeValue(ancestor);
          });
        } else if (this.autoDeselectDescendants) {
          this.traverseDescendantsBFS(node, function (descendant) {
            if (_this20.isSelected(descendant) && !descendant.isDisabled) _this20.removeValue(descendant);
          });
        }
        return;
      }
      ...
      if (node.isBranch) {
        this.traverseDescendantsDFS(node, function (descendant) {
          if (!descendant.isDisabled || _this20.allowSelectingDisabledDescendants) {
            _this20.removeValue(descendant);
